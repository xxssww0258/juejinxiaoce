...
class Juejin {
    ...
    copyToPWDDir(dirname) {
        ...
        const output = path.resolve(__dirname, 'dist', 'md') // 从 ./dist/md 中复制出来
        
        // 只复制md文件
        const fileList = fs.readdirSync(output)
    ...
    async mainPage() {
        ...
        const headers = {
            ...
            'Connection': 'keep-alive' // HTTP持久连接
        }
...
{(async () => {
    ...
    try {
        ...
        const dirname = 'md ' + + new Date() // 转时间戳
